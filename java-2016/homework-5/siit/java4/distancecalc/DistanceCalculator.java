...
public class DistanceCalculator {
	...
	public double stringConverter(String expression, String outputUnit) throws UnknownDistanceFormatException {
		...
		for (int i = 0; i < expresionArray.length; i++) {
			...
			if ((Character.toString(expresionArray[i]).matches("[*]"))
					|| (Character.toString(expresionArray[i]).matches("[/]"))
					|| (Character.toString(expresionArray[i]).matches("[_]"))
					|| (Character.toString(expresionArray[i]).matches("[)]"))
					|| (Character.toString(expresionArray[i]).matches("[(]"))) {
			...
			// This IF detects a number or a . in the array of characters and
			// makes a string of that numbers
			...
			// This IF detects a letter in the array of characters and makes a
			// string of that letters
			...
			// This IF detects once a group of numbers and letters have finished
			// (Ex: 12miles) and converts the numbers into a double,
			// the letters into a double based on the conversion class,
			// multiplies the two , and based on the operator sums or
			// substract's
			// with the next group
			if (i > 0) {
				if (!Character.isDigit(expresionArray[i]) && (!Character.isLetter(expresionArray[i]))
						&& (Character.isLetter(expresionArray[i - 1]))) {
					double result = 0;
					OtherUnitToMetricConverter conversion = new OtherUnitToMetricConverter();
					double numberTransformed = Double.parseDouble(number);
					number = null;
					double unitTransformed = conversion.unitConversionToMetric(unit);
					unit = null;
					result = numberTransformed * unitTransformed * firstNumberSign;
					firstNumberSign = 1;
					if (signCheck) {
			...
			// This IF detects an operator (+ or -) in the array of characters
			// and then put's that value in a string called operator
			if ((Character.toString(expresionArray[i]).matches("[+]"))
					|| (Character.toString(expresionArray[i]).matches("[-]"))) {
